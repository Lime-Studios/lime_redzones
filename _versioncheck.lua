    ...
    function CheckBridgeVersion()
        if IsDuplicityVersion() then
            CreateThread(function()
                Wait(4000)
                local currentVersionRaw = GetResourceMetadata("lime_redzones", 'version')
                PerformHttpRequest('https://raw.githubusercontent.com/Lime-Studios/lime_versions/refs/heads/main/redzones.txt', function(err, body, headers)
                    if not body then
                        print("^1Unable to run version check for ^7'^3lime_redzones^7' (^3"..currentVersionRaw.."^7)")
                        return
                    end
                    ...
                    if compareResult == 0 then
                        print("^7'^3lime_redzones^7' - ^2You are running the latest version^7. ^7(^3"..currentVersionRaw.."^7)")
                    elseif compareResult < 0 then
                        print("^1----------------------------------------------------------------------^7")
                        print("^7'^3lime_redzones^7' - ^1You are running an outdated version^7! ^7(^3"..currentVersionRaw.."^7 → ^3"..newestVersionRaw.."^7)")
                        ...
                        print("^7'^3lime_redzones^7' - ^5You are running a newer version ^7(^3"..currentVersionRaw.."^7 ← ^3"..newestVersionRaw.."^7) (^1Expect Errors^7)")
                    end
                end)
            end)
        end
